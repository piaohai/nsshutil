...
if (process.argv.length<=2){
  console.error('input ip file in data director');
  process.exit();
}

Config.prototype.init = function() {
  var config = this.config;
  var configStr=fs.readFileSync(__dirname+'/'+configFile,"utf8").toString().split("\n");;
  ...
  this.ips = [];
  var duplicate = {};
  configStr=fs.readFileSync('./data/'+ipFile,"utf8").toString().split("\n");;
  for (var i in configStr){
    var line = configStr[i];
    if (!!line){
      if (!duplicate[line]) {
